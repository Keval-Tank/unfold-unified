...
import RippleStaticFallback, {
  type StaticLotusLayout,
} from "@/components/RippleStaticFallback";
import type { LotusLayout } from "@/components/RippleWaterScene";
...
// Villa's lotus proportions. The ripple scene itself (shaders, physics, ~500
// lines) is shared verbatim with saffron; only these placements are villa's, so
// they travel as props rather than as a forked copy of the component.
const LOTUS: LotusLayout = {
  topRightWidthFrac: 0.28,
  lowerLeftWidthFrac: 0.2,
  lowerLeftXFrac: 0.1,
};

// The static fallback mirrors the live scene's placement, so the two look the
// same on devices that never get the WebGL canvas.
const STATIC_LOTUS: StaticLotusLayout = {
  topRight: { className: "pointer-events-none absolute right-0 top-24 z-10 w-[28%]" },
  lowerLeft: { className: "pointer-events-none absolute left-0 top-[55%] z-10 w-[20%]" },
};

/** One timeline row, exactly as this section draws it. The ordinal suffix is
 *  already applied by the caller (via eventDate()) — this section derives
 *  nothing, it only renders. */
...
// A CLIENT component: it pins itself with GSAP and hosts the WebGL ripple scene.
// Every string it draws arrives as a prop.
export default function TimelineSection({
  ...
  // Read per render (NOT at module scope) so events added or removed in the live
  // editor actually reach the pinned-scroll setup below.
  const EVENTS = events;
  ...
  // Rebuilds on event-COUNT change so an event added or removed in the editor is
  // included in the pinned-scroll setup — the ScrollTrigger distance, the dot
  // offsets and the per-dot reveal loop all key off the count. Text/date edits
  // don't change the count and re-render through JSX without re-running this.
  useEffect(() => {
    const section = sectionRef.current;
    if (!section) return;

    // Resize the reveal latch to the new count (its useRef initial value is only
    // evaluated on mount, so it would otherwise keep the old length).
    wasReachedRef.current = EVENTS.map((_, i) => i === 0);
    ...
    return () => ctx.revert();
    // Deliberately keyed on the COUNT, not on EVENTS itself: a new array
    // identity every render would rebuild the pin on every keystroke.
  ...
  return (
    ...
    <section
      ...
      style={{
        ...
        // fountain divider below. So no device height can push the last event
        // under the divider; 130svh only ADDS room on tall screens (preserving
        ...
        //              automatically as events are added to content.timeline
        //   52cqw    — bottom fountain divider reserve; scales with frame width
        //              so the last event clears it on any screen width
        minHeight: `max(130svh, calc(2rem + ${TIMELINE_EVENT_COUNT} * 10rem + 52cqw))`,
    ...
    >
      ...
      {rippleEnabled && !committedFallback && (
        <RippleErrorBoundary
          ...
          fallback={<RippleStaticFallback lotus={STATIC_LOTUS} />}
        >
          ...
          <RippleWaterScene revealed={rippleCommitted} onCommit={handleCommit} lotus={LOTUS} />
        </RippleErrorBoundary>
      )}
      ...
      {!rippleCommitted && <RippleStaticFallback lotus={STATIC_LOTUS} />}
      ...
      <div
        ...
        style={{
          ...
          bottom: 0,
          width: "1px",
          // Dashed track: a repeating gradient keeps the exact 1px geometry
          // (so the solid progress line still overlays perfectly) while
          // rendering as clean 4px dashes with 4px gaps.
          backgroundImage:
            "repeating-linear-gradient(to bottom, var(--color-white-muted) 0, var(--color-white-muted) 4px, transparent 4px, transparent 8px)",
        }}
      />
      ...
      <div className="pointer-events-none relative z-20 mx-auto flex w-full flex-1 flex-col justify-start gap-24 pb-60  pl-14 pr-8 md:pb-78">
        {EVENTS.map((event, i) => (
          <div key={i} className="relative flex items-start gap-3 mt-5">
            {/* Date block — rendered for any event whose data carries a
                non-empty `date`. Sits in the left gutter (pl-14) to the LEFT
            ...
            {event.date && (
              ...
              >
                ...
                <div
                  className="mt-1 font-manrope uppercase tracking-widest leading-none"
            ...
            <div className="relative flex-1 min-w-0 mt-5">
              ...
              >
                <p
                  className="mt-1 text-start font-manrope leading-normal"
                  style={{
                    color: "var(--color-white)",
                ...
                <p
                  className="mt-1.5 flex items-center gap-1.5 font-manrope tracking-widest uppercase text-center"
                  style={{
                    color: "var(--color-white)",
      ...
      {/* Bottom-seam fountain divider — INSIDE the section so it gets pinned with it. Clearance to the last event is guaranteed by the section's count-scaled min-height (see the `minHeight` on <section> above), which grows with the event count and reserves ~52cqw at the bottom for this divider. */}
      <div className="pointer-events-none absolute -bottom-30 left-0 right-0 z-30">
        <Image
          src={asset("/images/eu-fountain.webp")}
          alt=""
          width={768}
          height={527}
          sizes="100vw"
