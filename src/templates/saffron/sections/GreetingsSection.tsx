...
// A SERVER component — pure markup, no hooks, no browser APIs. Its copy is
// rendered to HTML on the server; ScrollReveal is a client component but only
// receives already-rendered children, so no content travels as props.
export default function GreetingsSection({
  ...
  return (
    <section
      ...
      style={{ backgroundImage: `url('${asset("/images/watercolor-landscape.webp")}')` }}
    >
      ...
      {/* Greeting + names — nudged above center so the empty top space reads as
          intentional, with a sequential fade: message → bride → & → groom */}
      <div className="relative z-10 flex w-full flex-col items-center -translate-y-[10vh] sm:translate-y-[vh]">
        <ScrollReveal animation="fadeInUp">
          <p
            className="max-w-60 font-montserrat italic leading-relaxed text-center"
            style={{
        ...
        <ScrollReveal animation="fadeInUp" delay={0.65}>
          <p
            className="mt-15 font-montserrat font-semibold uppercase tracking-[0.25em]"
            style={{
